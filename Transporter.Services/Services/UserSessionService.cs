...
#pragma warning disable CS8600
    public class UserSessionService : IUserSessionService
    {
        ...
        public UserSessionService(SbDbContext ctx)
        {
        ...
        public async Task<ResponseMessage> GetAllUserSession(RequestMessage requestMessage)
        {
            ...
            try
            {
                ...
                lstUserSession = await _crmDbContext.UserSession.OrderBy(x => x.UserSessionID).Skip(totalSkip).Take(requestMessage.PageRecordSize).ToListAsync();
        ...
        public async Task<ResponseMessage> GetUserSessionById(RequestMessage requestMessage)
        {
            ...
            try
            {
                ...
                objUserSession = await _crmDbContext.UserSession.FirstOrDefaultAsync(x => x.UserSessionID == UserSessionID && x.Status == (int)Enums.Status.Active);
        ...
        public async Task<ResponseMessage> GetUserSessionBySystemUserId(RequestMessage requestMessage)
        {
            ...
            try
            {
                ...
                objUserSession = await _crmDbContext.UserSession.AsNoTracking().OrderBy(u => u.UserSessionID).LastOrDefaultAsync(x => x.SystemUserID == systemUserId && x.Status == (int)Enums.Status.Active && x.SessionEnd > DateTime.Now);
                responseMessage.ResponseObj = objUserSession;
                responseMessage.ResponseCode = (int)Enums.ResponseCode.Success;
            ...
            try
            {
                ...
                if (objUserSession != null)
                {
                    if (CheckedValidation(objUserSession, responseMessage))
                    {
                        ...
                        {
                            UserSession existingUserSession = await this._crmDbContext.UserSession.AsNoTracking().FirstOrDefaultAsync(x => x.UserSessionID == objUserSession.UserSessionID && x.Status == (int)Enums.Status.Active);
                            if (existingUserSession != null)
                            {
                                ...
                                _crmDbContext.UserSession.Update(objUserSession);
                        ...
                        else
                        {
                            ...
                            await _crmDbContext.UserSession.AddAsync(objUserSession);
